...
def main():
    ...
    print("=== Standard 5-Fold CV (KFold, no grouping) ===")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X), start=1):
        ...
        high_ge_th = fi_pct[fi_pct >= LOW_TH]
    ...
    imp_summary = pd.DataFrame({
        ...
        "folds_ge_80": (imp_mat >= LOW_TH).sum(axis=1)  # 名字保留（其實是 >= LOW_TH）
    ...
    mean_ge_th = imp_summary[imp_summary["importance_mean_%"] >= LOW_TH]
    print("None" if len(mean_ge_th) == 0 else mean_ge_th.to_string(index=False, formatters={
        "importance_mean_%": "{:.2f}".format,
        "importance_std_%": "{:.2f}".format,
    }))
    ...
    # ==========================================================
    # 10) 分析：importance >= LOW_TH 的特徵 與 Y 的對應關係（含趨勢線/方程式/r）
    # ==========================================================
